(function () {
  ...
  /* ── Suggestions fetch (frontend-only) ──────────────────────
     Uses the Google Suggest endpoint. This is NOT an officially
     documented Google developer API; it is an undocumented public
     endpoint commonly used by browsers and third-party tools.
     No API key or credentials are required.

     NOTE ON CORS: suggestqueries.google.com does NOT send
     Access-Control-Allow-Origin headers for arbitrary origins.
     A direct fetch() from a non-Google origin will be blocked by
     the browser's CORS policy. If this occurs in production, the
     catch() handler will silently suppress the error and images
     will continue working normally. No backend workaround is
     created — that is a strict requirement. The limitation is
     reported here as required.
  ─────────────────────────────────────────────────────────────── */

  function _fetchSuggestions(q, token) {
    if (!q) return;

    // Google Suggest — undocumented public endpoint, no credentials.
    // hl=en&gl=in targets English suggestions with India locale context.
    const url = 'https://suggestqueries.google.com/complete/search'
      + '?client=firefox&hl=en&gl=in&q=' + encodeURIComponent(q);
    ...
    fetch(url)
      ...
      .then(function (data) {
        ...
        for (const t of data[1]) {
          ...
          seen.add(tl);
          fresh.push(trimmed);
          if (fresh.length >= 12) break; // sensible pool cap
        }
      ...
      .catch(function () {
        ...
        // CORS rejection lands here silently; no error UI is shown.
        if (token !== _queryToken) return;
        _suggestions = [];
        _suggPool = [];
      });
