...
const KB_CLOSE_THRESH = 8;
const KB_STREAM_GAP   = 180;
...
function _commitViewport(now) {
  ...
  const needsSpacerWrite = spacerChanged || kbEdge || transformChanged;

  if (!transformChanged && !needsSpacerWrite) {
    _kbOpen      = kb.kbOpen;
  ...
  if (needsSpacerWrite && chatSpacer) {
  ...
  if (transformChanged) {
    ...
    const animInFlight         = _barAnimating;
...
if (pill && input) {
  pill.addEventListener('pointerdown', (e) => {
    ...
    if (document.activeElement === input || _kbOpen || _kbArmed || _lastKbInset > 0) return;
