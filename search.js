...
/* ════════════════════════════════
   SSE STREAM PARSER
   ════════════════════════════════
   FIX: empty lines between SSE fields must NOT reset eventType.
   Per SSE spec, only a blank line (dispatch) marks the end of an
   event block — and by that point we've already processed `data:`.
   Resetting eventType on every non-data line was swallowing the
   `map`, `stock`, and `results` payloads entirely.
   ════════════════════════════════ */

async function _parseSseStream(reader, onResults, onDelta, onStock, onMap) {
  ...
  while (!done) {
    ...
    for (const line of lines) {
      // `event:` field — store for the upcoming `data:` line
      if (line.startsWith('event: ')) {
        eventType = line.slice(7).trim();
        continue;
      }

      // Blank line = SSE block separator — reset eventType for next block
      if (line.trim() === '') {
        eventType = '';
        continue;
      }

      // Skip any non-data field (comments, id:, retry:) without touching eventType
      ...
      if (eventType === 'map') {
        try { const m = JSON.parse(data); if (m) onMap(m); } catch (_) {}
        eventType = '';
        continue;
      }
      ...
      // No eventType = regular streaming text delta
      try {
...
async function send() {
  ...
  /* NOTE: _tryRenderMapCard (maps-card.js) is intentionally NOT called here.
     Map data arrives via the SSE `event: map` stream from /api/chat, which
     already calls Geoapify internally. Calling _tryRenderMapCard as well would
     make a redundant second Geoapify request and render a duplicate map card. */
  ...
  try {
    ...
    const fullText = await _parseSseStream(
      ...
      (mapData) => {
        /* Map card via SSE — single source of truth, no duplicate API call */
        if (!mapShown && typeof _renderMapCard === 'function') {
          removeTyping();
          _renderMapCard(mapData, sessionStorage.getItem('atkyn_last_query') || '');
          showTyping();
          mapShown = true;
        }
      }
