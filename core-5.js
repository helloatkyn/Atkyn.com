...
if (window.matchMedia) {
  ...
  /* ── Theme-freeze reconciliation timer ── */          // ← CHANGED: new state var
  let _themeReconcileTimer = 0;                          // ← CHANGED

  const _onThemeChange = () => {
    // ← CHANGED: extended from 350 → 500 ms to cover slower Android repaints
    _themeFreezeUntil = performance.now() + 500;
    ...
    // ← CHANGED: cancel any pending post-freeze reconciliation from a prior
    //   theme change so only the latest one fires.
    if (_themeReconcileTimer) {
      clearTimeout(_themeReconcileTimer);
      _themeReconcileTimer = 0;
    }
    ...
    _themeRefreshRaf = requestAnimationFrame(() => {
      _themeRefreshRaf = requestAnimationFrame(() => {
        ...
        /*
         * ── CHANGED ──────────────────────────────────────────────────────
         * Previously this block only preserved the snapshot when freshKb===0.
         * When interactive-widget=resizes-content is active, window.innerHeight
         * already excludes the keyboard, so rawKb is always ≈0 regardless of
         * whether the keyboard is open or closed.  That means freshKb is 0
         * even when the keyboard IS open, causing the old else-branch to call
         * _applyViewport(true) which computed kbHeight=0 and snapped the
         * chatbar to translateZ(0) — behind the keyboard.
         *
         * Fix: if the keyboard was open before the theme change (kbSnapshot>0),
         * ALWAYS reapply the snapshot transform and skip _applyViewport(true).
         * A real keyboard-close will arrive via the normal vvp 'resize' path
         * once the freeze window expires; a post-freeze reconciliation below
         * catches the case where the keyboard genuinely closed during the freeze.
         * ─────────────────────────────────────────────────────────────────
         */
        if (kbSnapshot > 0) {
          /*
           * Keyboard was open before the theme change.  Preserve the valid
           * keyboard state — do NOT call _applyViewport(true) here because
           * the rawKb formula returns 0 under resizes-content and would
           * incorrectly snap the chatbar to no offset.
           */
          ...
          // ← CHANGED: schedule a post-freeze reconciliation so that if the
          //   keyboard genuinely closed while we were frozen, the chatbar
          //   drops back to its correct position once geometry has settled.
          _themeReconcileTimer = window.setTimeout(() => {
            _themeReconcileTimer = 0;
            // Allow _applyViewport / fixViewport to run normally again.
            // _themeFreezeUntil has already expired by this point.
            if (vvp) {
              _applyViewport(true);
            }
          }, 520); // ← CHANGED: 520 ms > 500 ms freeze so geometry is stable

          return;
        }

        /*
         * Keyboard was already closed before the theme change.
