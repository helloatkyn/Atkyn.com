...
function _applyViewport() {
  ...
  if (_keyboardOpen && !wasOpen) {
    ...
    // Address-bar slide noise — keyboard state unchanged, suppress scroll jitter.
    _programmaticScroll = true;
  ...
  // When keyboard is closed, ignore any small kbHeight noise from
  // visualViewport.offsetTop fluctuating during address-bar hide/show.
  // Only apply a translateY when the keyboard is genuinely open (> 50px).
  const effectiveKb = _keyboardOpen ? kbHeight : 0;
  chatbarWrap.style.transform = effectiveKb > 0
    ? `translateY(-${effectiveKb}px) translateZ(0)` : '';
