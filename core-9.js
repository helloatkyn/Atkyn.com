...
function _applyViewport(opts) {
  ...
  if (!force && !transformChanged) {
    /* Pure entrance path: transform already correct, start opacity if not yet started. */
  ...
  /*
   * Entrance-only path (keyboard closed, no kb animation active).
   * Opacity animates; keyboard state stays closed; no kb lifecycle set.
   */
  ...
  if (_kbAnimating && !directionChanged) {
    /*
     * Same-direction update: only move the target transform.
     * Existing transition and listener remain authoritative.
     * The settle handler reads _kbAnimating at fire time — no stale capture.
     */
    chatbarWrap.style.transform = targetTransform;
  ...
  /*
   * New direction (or first animated move).
   * Replace previous lifecycle entirely: remove old listener, write new
   * transition once, attach one new listener.
   */
  ...
  /*
   * Settle handler: cleanup is unconditional on the transform property.
   * Does NOT compare against a captured target — the target may have been
   * updated by same-direction RAF calls; the transition end is authoritative
   * regardless of what the final value is.
   */
  const settle = (e) => {
