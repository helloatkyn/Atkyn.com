(function () {
  ...
  // One pill slot = 48px min-height + 9px gap-before = 57px. 72 for comfort margin.
  const MIN_GAP_PX = 72;
  ...
  function _buildTile(data, eager) {
    // thumbnail_src (Google-served cache) first — more reliable than img_src
    // which may redirect to site-specific placeholders. img_src is the fallback.
    const srcs = [];
    ...
    if (eager) {
      ...
      // visibility:hidden keeps image in render pipeline so load/error fire on
      // Android WebView where display:none suppresses events on lazily-set src.
  ...
  function _measureAndFill(gridSection) {
    ...
    // Each slot: 9px gap-before + 48px pill min-height = 57px.
    const pillSlotHeight = 48 + 9;
    const maxPills = Math.floor(gap / pillSlotHeight);
    if (maxPills < 1) return;

    shorterCol.querySelectorAll('.sugg-pill--gap').forEach(function (el) { el.remove(); });
