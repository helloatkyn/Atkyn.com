...
function renderMarkdown(rawText) {
  ...
  const html = text.split('\n\n').map(block => {
    ...
    /* Unordered list — CODE placeholders on their own line break out cleanly */
    if (/^[-*•] /m.test(t)) {
      const lines = t.split('\n').filter(Boolean);
      let out = '';
      let buf = [];
      ...
      for (const l of lines) {
        const cm = l.trim().match(/^\x00CODE(\d+)\x00$/);
    ...
    /* Paragraph (default) */
    // A paragraph block may contain standalone CODE placeholders on their own
    // line (e.g. "**Python:**\n\x00CODE0\x00"). Split those out so they render
    // as proper code blocks rather than leaking as "CODE0" text.   [F23]
