...
function _renderStockCard(data) {
  ...
  const newsHtml = data.news?.length
    ? data.news.map(n => `
        <a class="sc-news-item" href="${escapeHtml(n.url)}" target="_blank" rel="noopener noreferrer">
          <span class="sc-news-source">${escapeHtml(n.source)}</span>
          <span class="sc-news-headline">${escapeHtml(n.headline)}</span>
        </a>`).join('')
    : '';

  const logoHtml = data.logo
    ? `<img class="sc-logo" src="${escapeHtml(data.logo)}" alt="" onerror="this.remove()">`
    : '';

  function _toTvExchange(ex) {
    if (!ex) return '';
    const u = ex.toUpperCase();
    if (u.includes('NASDAQ')) return 'NASDAQ';
    if (u.includes('NEW YORK') || u.includes('NYSE')) return 'NYSE';
    if (u.includes('AMEX') || u.includes('AMERICAN')) return 'AMEX';
    if (u.includes('OTC') || u.includes('PINK')) return 'OTC';
    return '';
  }
  const _tvPrefix = _toTvExchange(data.exchange);
  const tvSymbol  = _tvPrefix ? `${_tvPrefix}:${data.symbol}` : data.symbol;

  const chartId = `sc-tv-${data.symbol}`;
    ...
    <div class="stock-card">
      <div class="sc-header">
        <div class="sc-name-wrap">
          ${logoHtml}
          <div>
            <div class="sc-symbol">${escapeHtml(data.symbol)}</div>
            <div class="sc-name">${escapeHtml(data.name)}</div>
          </div>
        </div>
        <div class="sc-price-wrap">
          <div class="sc-price">$${fmt(data.price)}</div>
          <div class="sc-change" style="color:${color}">${arrow} ${sign}${fmt(data.change)} (${sign}${fmt(data.changePct)}%)</div>
        </div>
      </div>
      <div class="sc-chart-wrap"><div id="${chartId}"></div></div>
      <div class="sc-stats">
        <div class="sc-stat"><span class="sc-stat-label">Open</span><span class="sc-stat-val">$${fmt(data.open)}</span></div>
        <div class="sc-stat"><span class="sc-stat-label">High</span><span class="sc-stat-val">$${fmt(data.high)}</span></div>
        <div class="sc-stat"><span class="sc-stat-label">Low</span><span class="sc-stat-val">$${fmt(data.low)}</span></div>
        <div class="sc-stat"><span class="sc-stat-label">Prev Close</span><span class="sc-stat-val">$${fmt(data.prevClose)}</span></div>
        <div class="sc-stat"><span class="sc-stat-label">Mkt Cap</span><span class="sc-stat-val">${fmtCap(data.marketCap)}</span></div>
        <div class="sc-stat"><span class="sc-stat-label">P/E</span><span class="sc-stat-val">${fmt(data.pe)}</span></div>
        <div class="sc-stat"><span class="sc-stat-label">52W High</span><span class="sc-stat-val">$${fmt(data.week52High)}</span></div>
        <div class="sc-stat"><span class="sc-stat-label">52W Low</span><span class="sc-stat-val">$${fmt(data.week52Low)}</span></div>
      </div>
      ${newsHtml ? `<div class="sc-news">${newsHtml}</div>` : ''}
    </div>`;
