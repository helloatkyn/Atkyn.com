/* ═══════════════════════════════════════════════════════════════
   ...
   AUDIT FIXES (v3):
   ─────────────────
   [F21] PLACEHOLDER COLLISION FIX — root cause of CODE0/lang=0/class=0
         corruption. Old sentinel \x01N\x01 allowed numLit regex
         /\b\d+\b/ to match the digit N inside the sentinel, wrapping
         it in a new protect() call and permanently corrupting the
         original placeholder. Fix: sentinel changed to \x02«N»\x02
         where « and » (U+00AB / U+00BB) are non-ASCII, non-word-
         boundary characters that can never be matched by any
         tokenizer regex (\b, \w, \d, \s). All creation and
         restoration sites updated consistently.
   [F22] Math block-level sentinel \x00M / \x01DISPLAY also audited —
         \x00 and \x00 are control chars never matched by tokenizers;
         these were already safe and are left unchanged.
   [F23] Code block sentinel \x00CODE audited — lives entirely in the
         markdown layer, never enters _tokenizeLine; already safe.
   ═══════════════════════════════════════════════════════════════ */
 ...
 *
 * Sentinel \x00M{n}\x00 uses NUL (\x00) which is:
 *   - never produced by any LLM output
 *   - never matched by \w, \d, \b, \s or any tokenizer regex
 *   - safe to carry through the markdown layer (never enters _tokenizeLine)
 */
 ...
 *
 * PLACEHOLDER SYSTEM [F21]:
 * ─────────────────────────
 * Sentinel format:  \x02«N»\x02
 *   \x02  STX control character — never in LLM output, never matched by any
 *         JS regex character class (\w, \d, \s, \b, [a-z], [A-Z], [0-9])
 *   «»    U+00AB / U+00BB LEFT/RIGHT-POINTING DOUBLE ANGLE QUOTATION MARK
 *         Non-ASCII, non-word characters — \b word boundary never fires here,
 *         so numLit /\b\d+\b/ can NEVER reach the digit N inside the sentinel.
 *   N     The array index (decimal digits). Safe because they are surrounded
 *         by the non-word «» chars which prevent \b from matching.
 *
 * This completely eliminates the collision where old sentinel \x01N\x01
 * allowed numLit to match N and wrap it in a new protect() call, corrupting
 * the placeholder and leaking CODE0 / lang=0 / class=0 into the output.
 *
 * All protect() calls and the final restore regex use this same format.
 * No other format exists in this function. No mixed sentinels.
 */
function _tokenizeLine(line, lang) {
  ...
  /*
   * protect(html) — stash a finished HTML span and return a sentinel token.
   * Sentinel: \x02«INDEX»\x02
   * \x02 = STX, non-word. «» = non-ASCII, non-word. Index digits are flanked
   * by non-word chars so \b\d+\b can never fire inside a sentinel.         [F21]
   */
  const protect = html => {
    const idx = ph.length;
    ph.push(html);
  ...
  /* span — escape raw source text and wrap it, then protect the result. */
  const span = (cls, text) => protect(`<span class="${cls}">${_esc(text)}</span>`);

  let s = line;

  /* ── 1. Block comments (multi-char, must come first) ── */
  ...
  /* ── 3. String literals (protect before HTML-escaping so quotes survive) ── */
  s = s.replace(_RX.btStr,  m => span('tk-str', m));
  s = s.replace(_RX.dqStr,  m => span('tk-str', m));
  s = s.replace(_RX.sqStr,  m => span('tk-str', m));

  /*
   * ── 4. HTML-escape the remaining raw source text ──
   * Sentinel tokens contain only \x02, «, », and decimal digits.
   * _esc() replaces only &, <, >  — none of those chars appear in sentinels,
   * so sentinels survive _esc() completely intact.
   */
  s = _esc(s);

  /*
   * ── 5. Numeric literals ──
   * numLit regex: /\b\d+(\.\d+)?\b/g
   * Because every existing sentinel is \x02«N»\x02, the digit N is flanked
   * by the non-word char » on the left and \x02 on the right. \b fires only
   * between a \w and a \W character; » and \x02 are both \W, so \b never
   * fires adjacent to a sentinel digit. numLit therefore matches ONLY actual
   * source-code digits, never sentinel indices.                             [F21]
   */
  s = s.replace(_RX.numLit, m => protect(`<span class="tk-num">${m}</span>`));

  /* ── 6. Class names, function calls, keywords, booleans, prop keys ── */
  ...
  /*
   * ── 7. Restore all sentinels ──
   * Pattern matches \x02«DIGITS»\x02 and replaces with ph[index].
   * No other pattern in this function shares this shape.                   [F21]
   */
  s = s.replace(/\x02\u00AB(\d+)\u00BB\x02/g, (_, i) => ph[+i]);
...
function renderMarkdown(rawText) {
  ...
  // First pass: strip leading hashes from heading lines into clean canonical form.
  // Handles: "### # text", "### ## Emoji text", "#### **text**", etc.
  // Strategy: capture the leading #+ block, then clean up the rest of the line.
  text = text.replace(/^(#{1,6})\s+([\s\S]*?)$/gm, (_, hashes, rest) => {
    const level = Math.min(hashes.length, 4); // cap at ####
    // Strip any extra leading #+ that the LLM appended after the space
    let clean = rest.replace(/^#+\s*/, '');
    // Strip surrounding ** if LLM bolded the heading text
    clean = clean.replace(/^\*\*(.+?)\*\*$/, '$1').trim();
    return '#'.repeat(level) + ' ' + clean;
  });
  // Second pass: promote lone # to ## (same visual level in this app)
  ...
  // Sentinel \x00CODE{n}\x00 uses NUL — never in LLM output, never enters
  // _tokenizeLine, cannot collide with any tokenizer regex.
  ...
  const html = text.split('\n\n').map(block => {
    ...
    /* Paragraph (default) */
    // [F20] Lines within a paragraph are joined with a space, not <br>.
    // This prevents prose wrapped by the LLM at arbitrary column widths
    // from rendering as visually broken lines.
    // Exception: a line ending with two or more spaces is an explicit
    // hard-break (standard Markdown spec).
