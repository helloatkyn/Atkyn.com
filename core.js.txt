...
const pillWrap     = document.getElementById('pillWrap');   /* ← gradient border wrapper */
...
/* ── Animated gradient border: focus/blur on pillWrap ── */
if (pillWrap) {
  input.addEventListener('focus', () => pillWrap.classList.add('focused'));
  input.addEventListener('blur',  () => pillWrap.classList.remove('focused'));
}
