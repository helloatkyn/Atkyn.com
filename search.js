...
/* _applyKatex — intentionally empty.
   renderer.js handles all math ($$, \[...\], \(...\)) at parse time.
   Single-dollar $...$ is disabled globally — causes false positives with prices.
   auto-render CDN script is not needed and can be removed from HTML. */
function _applyKatex(_el) { /* no-op */ }
...
function addMsg(role, text) {
  ...
  if (role === 'user') {
    // User text — plain text only, never rendered as markdown/math.
    // Prevents $price from being misread as LaTeX delimiters.
