...
function _applyViewport() {
  ...
  /* Pinch-zoom guard: jab user zoom kare tab chatbar ko
     visualViewport ke andar rakh — fixed element float na kare */
  if (vvp.scale > 1.01) {
    const offsetY = vvp.offsetTop + vvp.height - chatbarWrap.offsetHeight;
    chatbarWrap.style.transition = 'none';
    chatbarWrap.style.position   = 'absolute';
    chatbarWrap.style.top        = offsetY + 'px';
    chatbarWrap.style.bottom     = '';
    return;
  }

  /* Zoom nahi hai — position reset karo agar badla tha */
  if (chatbarWrap.style.position === 'absolute') {
    chatbarWrap.style.position = '';
    chatbarWrap.style.top      = '';
    chatbarWrap.style.bottom   = '';
  }
...
if (window.visualViewport) {
  ...
  window.visualViewport.addEventListener('scale',  fixViewport, { passive: true });
  updateSpacer(0);
  _applyViewport();
} else {
  function _legacyFix() {
