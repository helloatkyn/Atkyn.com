...
/* Chrome-style scroll-linked tab hide:
   - No binary snap — translateY tracks scroll delta 1:1
   - Max hide = 65% of tabBar height (chips never fully disappear)
   - No CSS transition on scroll — only on programmatic snap-back
   - _tabOffset: accumulated hide offset [0 .. MAX_TAB_HIDE] */
let _tabOffset = 0;

function _applyTabOffset(offset, animated) {
  tabBar.style.transition = animated ? 'transform 0.22s ease-out' : 'none';
  tabBar.style.transform  = offset > 0 ? `translateY(-${offset}px) translateZ(0)` : '';
}
...
function updateHeader() {
  ...
  /* Content scroll nahi ho sakta — tab ko touch hi mat karo */
  if (scrollHost.scrollHeight <= scrollHost.clientHeight + 2) {
    if (_tabOffset !== 0) { _tabOffset = 0; _applyTabOffset(0, true); }
    return;
  }
  ...
  /* At very top: snap everything back cleanly */
  if (sy <= LOGO_THRESH) {
    _tabOffset = 0;
    _applyTabOffset(0, true);
  ...
  /* Scroll-linked 1:1 tracking — clamped to [0, 65% of tabBar height] */
  const maxHide = Math.floor(tabBar.offsetHeight * 0.65);
  _tabOffset = Math.min(maxHide, Math.max(0, _tabOffset + delta));
  _applyTabOffset(_tabOffset, false);
}
