...
/* Theme refresh generation counter.
   Incremented on every prefers-color-scheme change.
   VVP RAFs that fire while a theme refresh is pending yield
   to the theme RAF to avoid applying intermediate geometry. */
let _themeGen        = 0;
let _themeRafPending = false;
...
function fixViewport() {
  ...
  /* Yield to a pending theme refresh: theme RAF2 will call _applyViewport()
     with fully-settled post-theme VVP geometry. An intermediate VVP event
     here would only commit a transient measurement. */
  if (_themeRafPending) return;
  _vvpDirty = true;
...
if (chatbarWrap && typeof ResizeObserver === 'function') {
  new ResizeObserver((entries) => {
    ...
    /* Read live VVP geometry — never trust _stableKbH here, it may be
       transiently stale if ResizeObserver fires between a VVP RAF and
       a theme refresh RAF. */
    const { keyboardInset: liveInset } = getViewportState();
    _setSpacerHeight(_barHeight + liveInset);
  }).observe(chatbarWrap);
}
...
if (window.matchMedia) {
  ...
  const _onThemeChange = () => {
    const gen = ++_themeGen;
    _themeRafPending = true;

    /* Cancel any pending viewport RAF — we take ownership of the next measurement. */
    if (_vvpDirty) {
    ...
    /*
     * Two RAFs:
     *   RAF1 — browser applies theme colors + layout recalc.
     *   RAF2 — VVP has settled to post-theme geometry on Android Chrome.
     * Generation token ensures that if another theme change fires while
     * these are pending, only the latest callback applies geometry.
     */
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        if (gen !== _themeGen) return;   /* superseded by a newer theme change */
        _themeRafPending = false;
