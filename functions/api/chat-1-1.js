...
const MAX_TOKENS_TOOL_SELECT   = 150;   // Call 1: tool routing — only needs a tool call
const MAX_TOKENS_DIRECT_ANSWER = 600;   // Call 1: direct answer (no tool needed)
const MAX_TOKENS_ANSWER        = 350;   // Call 2: final answer after tool execution
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!call1Resp.ok) {
      ...
      if (msg?.tool_calls?.length) {
        ...
        // If the direct answer was cut off due to the low token budget, retry with
        // a higher limit so the full response is returned to the user.
        if (reason === 'length') {
          const retryResp = await mistralFetch(env.MISTRAL_API_KEY, {
            model:               MODEL_TOOL_SELECT,
            messages:            call1Messages,
            tools:               TOOLS,
            tool_choice:         'auto',
            parallel_tool_calls: false,
            stream:              false,
            max_tokens:          MAX_TOKENS_DIRECT_ANSWER,
            temperature:         0.2,
          });
          if (retryResp.ok) {
            const rd      = await retryResp.json().catch(() => null);
            const retryMsg = rd?.choices?.[0]?.message;
            if (retryMsg?.tool_calls?.length) {
              toolCall = retryMsg.tool_calls[0];
              firstAssistMsg = retryMsg;
            } else if (typeof retryMsg?.content === 'string' && retryMsg.content.trim()) {
              directAnswer   = retryMsg.content.trim();
              firstAssistMsg = retryMsg;
            }
          } else {
            // Retry failed — use the truncated answer rather than nothing
  ...
  (async () => {
    ...
    try {
      ...
      let buffer = '';
      // Regex to strip any XML-style tool call blocks the model may emit as text
      // (e.g. Qwen emits <tool_call>...</tool_call> instead of native tool_calls)
      ...
      while (true) {
        ...
        if (done) {
          ...
          if (buffer) {
            const cleaned = buffer.replace(TOOL_CALL_TAG_RE, '').trim();
            if (cleaned) {
              await safeWrite(enc.encode(cleaned));
            }
          }
          break;
        }

        // Accumulate decoded text to handle tags that may span chunk boundaries
        buffer += decoder.decode(value, { stream: true });

        // Only flush when we're sure we're not in the middle of a tag
        // Keep a tail that could be a partial opening tag
        const SAFE_TAIL = 50; // max length of '<tool_call' prefix
        if (buffer.length > SAFE_TAIL) {
          const safeEnd = buffer.length - SAFE_TAIL;
          const safeChunk = buffer.slice(0, safeEnd);
          buffer = buffer.slice(safeEnd);

          // Strip complete tool_call blocks from the safe portion
          const cleaned = safeChunk.replace(TOOL_CALL_TAG_RE, '');
          if (cleaned) {
            await safeWrite(enc.encode(cleaned));
          }
        }
