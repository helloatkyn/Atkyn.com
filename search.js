...
function _applyViewport() {
  ...
  _cleanupRafId = requestAnimationFrame(() => {
    _cleanupRafId = 0;
    // resetScrollAccum first so updateHeader sees clean state before _lastScrollY update.
    resetScrollAccum();
    _lastScrollY = scrollHost.scrollTop;
...
if (window.visualViewport) {
  ...
  // scroll listener removed: visualViewport.scroll fires on normal page scroll too,
  // which was incorrectly setting _programmaticScroll=true mid-scroll and causing
  // the search bar to bounce (delta flip in updateHeader).
  updateSpacer(0);
  _applyViewport();
} else {
  function _legacyFix() {
...
// HIDE_ACCUM: px of intentional downward scroll before tab-bar hides.
// 80px avoids accidental hide on small scrolls / momentum wobble.
// SHOW_ACCUM: px of upward scroll before tab-bar shows — slightly higher
// than HIDE_ACCUM so it doesn't flash back on a tiny reverse flick.
// LOGO_THRESH: scrollTop must exceed this before logo stays collapsed.
// 30px gives enough runway that momentum bounce doesn't re-expand it.
const HIDE_ACCUM  = 80;
const SHOW_ACCUM  = 60;
const LOGO_THRESH = 30;
...
function updateHeader() {
  ...
  // Collapse logo in sync with tab-bar hide so they move together.
  // Previously logo collapsed instantly on any scroll past LOGO_THRESH,
  // causing a mismatch with the accumulator-gated tab-bar.
  ...
  if (delta > 0) {
    ...
    if (!_isTabHidden && _accumDown >= HIDE_ACCUM) {
      tabBar.classList.add('hide');         _isTabHidden     = true;
    ...
    if (_isTabHidden && _accumUp >= SHOW_ACCUM) {
      tabBar.classList.remove('hide');      _isTabHidden     = false;
      if (_isLogoCollapsed) { logoHeader.classList.remove('collapsed'); _isLogoCollapsed = false; }
      _accumUp = 0;
    }
  }
