(function () {
  ...
  function _loadImage(image) {
    ...
    image.src = src;
    _preloaded.add(src);
    // If the browser fulfilled the request synchronously from cache, the load
    // event fires before this line and onload has already run. If complete is
    // true here but loaded is still unset, the event was missed; call onload
    // directly so the intrinsic-ratio correction is never skipped.
  ...
  function _buildTile(data) {
    ...
    image.onload = function () {
      ...
      // Replace the API-metadata-derived spacer height with the actual intrinsic
      // aspect ratio of the image that loaded (thumbnail_src may differ from the
      // original source dimensions stored in data.width / data.height).
      // This is the authoritative geometry: whatever URL actually loaded, use its
      // real naturalWidth × naturalHeight to size the card correctly.
