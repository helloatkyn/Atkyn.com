(function () {
  ...
  function _buildTile(data) {
    ...
    // Use display:none (not just visibility:hidden) so the browser never renders
    // the broken-image icon, which ignores visibility but respects display.
    ...
    image.onerror = function () {
      // Image failed: collapse reserved space immediately then remove the tile.
      // Collapsing first prevents the empty padded spacer from briefly showing.
      // display:none is already set, so the broken-image icon never appears.
      const spacer = this.parentElement;
      if (spacer && spacer.classList.contains('img-tile__spacer')) {
