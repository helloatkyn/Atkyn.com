/* ═══════════════════════════════════════════════════════════════
   ...
   AUDIT FIXES (v5 — UNDERSCORE ITALIC FIX):
   ──────────────────────────────────────────
   [F28] _fmt(): added underscore italic handler _text_ → <em>text</em>
         Mistral Large uses _text_ style italic which was unhandled,
         causing words to merge (e.g. "Applekarevenue...").
         Regex: /(?<![_\w])_(?!_)([^_\n]+?)(?<![_\w])_(?!_)/g
         Lone underscores not between word chars are stripped after.
   [F29] Step 4: removed two lines that prematurely stripped *italic*
         patterns at document level BEFORE _fmt() could process them.
         These were:
           .replace(/^\*([^*\n]+)\*$/gm, '$1')
           .replace(/(?<!\*)\*([^*\n]+?)\*(?!\*)/g, '$1')
         _fmt() already handles italic correctly — double processing
         was causing some *italic* spans to be stripped as plain text
         instead of being wrapped in <em>.
   ═══════════════════════════════════════════════════════════════ */
...
function _fmt(line, math) {
  ...
  // Underscore italic: _text_ — only match _ not adjacent to word chars   [F28]
  s = s.replace(/(?<![_\w])_(?!_)([^_\n]+?)(?<![_\w])_(?!_)/g, '<em>$1</em>');
  // Remove remaining lone underscores that aren't between word chars
  s = s.replace(/(?<!\w)_(?!\w)/g, '');
...
/* ════════════════════════════════════════════════════════════════════
   UNIVERSAL MESSAGE RENDERING PIPELINE  (v1)
   ════════════════════════════════════════════════════════════════════

   ADDITIONS over renderer.js base:
   ─────────────────────────────────
   [U01]  repairMarkdown()   — conservative markdown repair
            • ##Title        → ## Title
            • **bold**text   → **bold** text
            • Key:$value     → Key: $value  (financial/colon spacing)
            • Key point:-txt → Key point:\n- txt
            Context-aware: never splits CamelCase product names, tickers,
            URLs, version strings, etc.

   [U02]  repairMathFragments() — broken math reassembly
            Detects token-per-line fragments that look like math
            expressions and joins them back as $…$ inline math.

   [U03]  normalizeWhitespace() — sane paragraph spacing
            Collapses 3+ blank lines to 2, trims leading/trailing
            blank lines.  Never touches code/math regions.

   [U04]  autoLinkUrls() — raw URL → clickable <a>
            Applied AFTER markdown link processing so existing
            [text](url) links are not double-processed.

   [U05]  UniversalMessageRenderer class
            • render(content) → HTML string
            • start() / push(chunk) / finish() streaming API
            • rawContent always preserved unchanged
            • Memoizes last render; skips re-parse on identical input
            • Error-tolerant: falls back to <pre> on catastrophic failure

   [U06]  createStreamingRenderer(onUpdate, debounceMs)
            Factory for streaming use-cases.  Returns { push, finish }.
            Accumulates chunks, debounces renderMarkdown calls so the
            UI is not re-rendered on every single token.

   ════════════════════════════════════════════════════════════════════ */


/* ── [U01] Conservative Markdown Repair ────────────────────────────
 *
 * Rules applied in order.  Each rule is documented with its rationale
 * and a safe-guard list of strings it must NEVER corrupt.
 *
 * Safe identifiers that must pass through untouched:
 *   OpenAI  ChatGPT  iPhone  AAPL  NASDAQ:AAPL  API  JSON
 *   JavaScript  TypeScript  HTTP  HTTPS  IPv4  C++  Node.js
 *   $4.5T  $309.35  34.85x  0.35%  13.63%
 */
function repairMarkdown(text) {
  if (!text) return text;

  /* ── R01: Heading without space: ##Title → ## Title
   *
   * Match: line starting with 1-6 # chars immediately followed by a
   *        non-space, non-# character.
   * Guard: Do NOT add a space when the char after # is also # (that is
   *        already handled by the heading normalizer in renderMarkdown).
   */
  text = text.replace(/^(#{1,6})([^#\s])/gm, '$1 $2');

  /* ── R02: Bold/italic immediately followed by word chars
   *         **bold**text → **bold** text
   *         *italic*text → *italic* text
   * Guard: Do not touch **bold** followed by punctuation, spaces, or
   *        end-of-line — those are already correct.
   */
  text = text.replace(/(\*{1,2})(\*{0,1}\w)/g, (m, stars, after) => {
    // Only add space when we are at a closing marker followed by word char
    // i.e. the character BEFORE stars[0] closed a span and immediately
    // bumped into a word character.
    return m; // We address this at the closing side below.
  });
  // Closing **…**word  →  **…** word
  text = text.replace(/(\*\*[^*\n]+?\*\*)([a-zA-Z\u0900-\u097F])/g, '$1 $2');
  // Closing *…*word    →  *…* word   (only single *, not preceded by another *)
  text = text.replace(/(?<!\*)\*([^*\n]+?)\*(?!\*)([a-zA-Z\u0900-\u097F])/g,
    (_, inner, after) => `*${inner}* ${after}`);

  /* ── R03: Colon-dollar spacing for financial values
   *         "Market Cap:$4.5T" → "Market Cap: $4.5T"
   *         "Price:$309.35"    → "Price: $309.35"
   * Guard: NASDAQ:AAPL — the character after : is a letter, not a $,
   *        so this rule does NOT fire on tickers.
   *        "https://" — the : is followed by /, not $. SAFE.
   */
  text = text.replace(/([A-Za-z\u0900-\u097F0-9])\:\$/g, '$1: $');

  /* ── R04: Colon-letter spacing ONLY when a clear label pattern exists
   *         "PE Ratio:34.85x" → "PE Ratio: 34.85x"
   * Heuristic: "Word:" followed immediately by a digit (not URL/ticker
   *             context).  Safe-guard: do not touch NASDAQ:AAPL or
   *             http:// — the regex requires the char after : to be a digit.
   */
  text = text.replace(/([A-Za-z\u0900-\u097F])\:(\d)/g, '$1: $2');

  /* ── R05: Bullet immediately after colon on same line
   *         "Key point:-text" → "Key point:\n- text"
   * Guard: only fires when :-  appears (dash immediately after colon,
   *        no space).  The replacement preserves the colon.
   */
  text = text.replace(/\:\-([^\s])/g, ':\n- $1');

  /* ── R06: Bullet without space  "- text" that starts correctly is fine.
   *         Handles "-text" (no space) at line start → "- text"
   * Guard: Only at line start; ignores "---" (HR), inline hyphens,
   *        "C++" "-v" flags, version strings like "v1.0-beta".
   *        The regex requires a letter or digit right after the dash,
   *        but NOT preceded by another dash or digit (avoids ---).
   */
...
/* ── [U02] Broken Math Fragment Reassembly ──────────────────────────
 *
 * Some LLMs emit mathematics split across many lines, one token per line:
 *
 *   344.57
 *   (
 *   high
 ...
 *   315
 *
 * This function detects runs of 2-8 consecutive lines that collectively
 * look like a math expression and collapses them into $…$ inline math.
 *
 * Heuristics (conservative):
 *   • Line is a pure operator:  +  -  −  ×  ÷  /  *  =  ≈  <  >  ±
 *   • Line is a parenthesis:    (  )  [  ]
 *   • Line is a short number:   \d+(\.\d+)?  (up to 15 chars)
 *   • Line is a single math word: high  low  close  open  max  min  avg
 *   • A run qualifies when ≥1 line is an operator AND ≥1 is a number
 *
 * Guard: ordinary prose sentences are NEVER collapsed because they
 *   contain words longer than the "math word" list.
 */
const _MATH_OP_LINE    = /^[+\-\u2212\u00D7\u00F7\/\*=\u2248<>\u00B1]\s*$/;
const _MATH_PAREN_LINE = /^[\(\)\[\]]\s*$/;
const _MATH_NUM_LINE   = /^\d{1,15}(\.\d{1,10})?\s*$/;
const _MATH_WORD_LINE  = /^(high|low|close|open|max|min|avg|mean|sum|price|eps|pe|ratio)\s*$/i;
...
function repairMathFragments(text) {
  ...
  while (i < lines.length) {
    // Try to collect a candidate run starting at line i
    const run = [];
    let j = i;

    while (j < lines.length && j - i < 9) {
      const l = lines[j].trim();
      if (!l) break; // blank line ends a run
      const isMathLike =
        _MATH_OP_LINE.test(l) ||
        _MATH_PAREN_LINE.test(l) ||
        _MATH_NUM_LINE.test(l) ||
        _MATH_WORD_LINE.test(l);
      if (!isMathLike) break;
      run.push(l);
      j++;
    }

    const hasOp  = run.some(l => _MATH_OP_LINE.test(l));
    const hasNum = run.some(l => _MATH_NUM_LINE.test(l));

    if (run.length >= 2 && hasOp && hasNum) {
      // Join into inline math, normalise minus signs
...
/* ── [U03] Whitespace Normalization ────────────────────────────────
 *
 * • Collapse 3+ consecutive blank lines → 2 blank lines
 * • Remove leading/trailing blank lines from the whole text
 * • Preserve code blocks and math regions (already extracted as
 *   placeholders before this runs in the main pipeline)
 * • Preserve intentional indentation inside lines
 */
...
/* ── [U04] Auto-link Raw URLs ───────────────────────────────────────
 *
 * Applied to final HTML output AFTER markdown rendering.
 * Finds http(s) URLs that are not already inside an <a …> tag.
 *
 * Guard:
 *   • Only http:// and https:// — never javascript:, data:, etc.
 *   • Skips URLs already wrapped in <a href="…">
 *   • Truncates display label to 60 chars for readability
 */
function _autoLinkUrls(html) {
  // We only want to linkify URLs that are NOT already inside an href attr
  // or inside a tag.  Simple approach: split on existing <a> tags, linkify
  // the text parts, rejoin.
  const ANCHOR_SPLIT = /(<a\s[\s\S]*?<\/a>|<[^>]+>)/gi;
  const URL_RE = /\bhttps?:\/\/[^\s<>"')\]]+/gi;

  return html
    .split(ANCHOR_SPLIT)
    .map((chunk, idx) => {
      // Even-indexed chunks are plain text / non-anchor markup.
      // But we also skip anything that looks like a tag already.
      if (idx % 2 === 1) return chunk; // already an anchor or tag fragment
      if (chunk.startsWith('<')) return chunk;
      return chunk.replace(URL_RE, url => {
        const safe = /^https?:\/\//i.test(url) ? url : '#';
        const label = url.length > 60 ? url.slice(0, 57) + '…' : url;
        return `<a href="${safe}" target="_blank" rel="noopener noreferrer">${_he(label)}</a>`;
      });
    })
    .join('');
}
...
/* ── [U05] UniversalMessageRenderer ────────────────────────────────
 *
 * Main entry point for all text rendering in the application.
 *
 * Usage:
 *   const r = new UniversalMessageRenderer();
 *   const html = r.render(rawText, { role: 'assistant', streaming: false });
 *
 * Streaming usage:
 *   const r = new UniversalMessageRenderer();
 *   r.startStream();
 *   r.pushChunk(tokenChunk);   // call on each incoming token
 *   r.finishStream();
 *   const html = r.getHTML();
 *
 * Properties after render():
 *   r.rawContent      — original unmodified text (always preserved)
 *   r.renderedContent — HTML output
 */
class UniversalMessageRenderer {
  constructor() {
    this.rawContent      = '';
    this.renderedContent = '';
    this._lastInputHash  = null;  // memoization key
    this._streamBuf      = '';
    this._streamActive   = false;
  }

  /* ── Full one-shot render ── */
  render(content, { role = 'user', streaming = false } = {}) {
    // Always store raw unchanged
    this.rawContent = content;

    // Fast path: skip re-parse if content identical to last call
    const hash = _cheapHash(content);
    if (hash === this._lastInputHash && this.renderedContent) {
      return this.renderedContent;
    }
    this._lastInputHash = hash;

    this.renderedContent = _safePipeline(content, role);
    return this.renderedContent;
  }
  ...
  pushChunk(chunk) {
    if (!this._streamActive) this.startStream();
    this._streamBuf  += chunk;
    this.rawContent   = this._streamBuf;
    // Render the accumulated buffer so far (partial render)
    this.renderedContent = _safePipeline(this._streamBuf, 'assistant');
    return this.renderedContent;
  }

  finishStream() {
    this._streamActive = false;
    // Final full render of complete content
    this.renderedContent = _safePipeline(this._streamBuf, 'assistant');
    return this.renderedContent;
  }
...
/* ── [U06] Streaming renderer factory ──────────────────────────────
 *
 * For call-sites that don't want to manage the class lifecycle.
 *
 *   const { push, finish } = createStreamingRenderer(html => {
 *     chatBubble.innerHTML = html;
 *   }, 40 /* ms debounce *\/);
 *
 *   // On each token from the AI stream:
 *   push(token);
 *
...
function createStreamingRenderer(onUpdate, debounceMs = 40) {
  ...
  function _flush(final = false) {
    if (_timer) { clearTimeout(_timer); _timer = null; }
    const html = final ? renderer.finishStream() : renderer.getHTML();
    if (typeof onUpdate === 'function') onUpdate(html, { final });
  }
  ...
  return { push, finish, getRenderer: () => renderer };
}
 ...
 *   normalizeWhitespace
 *   → repairMarkdown      (conservative formatting repair)
 *   → repairMathFragments (fragment reassembly)
 *   → renderMarkdown      (existing v5 renderer)
 *   → _autoLinkUrls       (raw URL linkification)
 *
 * All stages are wrapped in try/catch; on failure the function
 * falls through to the next stage or returns a <pre> fallback.
 */
function _safePipeline(raw, role) {
  ...
  try {
    ...
    try { text = normalizeWhitespace(text); } catch (_) { /* continue */ }

    // Stage 2 — conservative markdown repair
    try { text = repairMarkdown(text); } catch (_) { /* continue */ }

    // Stage 3 — broken math fragment reassembly
    try { text = repairMathFragments(text); } catch (_) { /* continue */ }

    // Stage 4 — main markdown → HTML renderer
    let html = '';
    try {
    ...
    // Stage 5 — auto-link bare URLs in the final HTML
    try { html = _autoLinkUrls(html); } catch (_) { /* continue */ }

    return html;

  } catch (fatal) {
    // Absolute last resort — never crash the UI
    return `<pre class="render-fallback">${_he(raw)}</pre>`;
  }
}


/* ── Simple string hash for memoization ────────────────────────────
 * Not cryptographic — just fast duplicate detection.
 */
...
/* ── Public surface ─────────────────────────────────────────────────
 *
 * Convenience wrapper matching the spec's function-call interface:
 *
 *   UniversalMessageRenderer(content, role, streaming)
 *
 * Returns rendered HTML string.
 * Also exposes the class and factory for richer use-cases.
 */
function universalRender(content, role = 'user', streaming = false) {
  ...
  return r.render(content, { role, streaming });
}
