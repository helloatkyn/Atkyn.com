...
function _userBracketToLatex(str) {
  const TOKENS = ['\\frac','\\int','\\sum','\\sqrt','\\lim','\\prod',
    '\\cdot','\\cdots','\\times','\\infty','\\alpha','\\beta','\\gamma',
    '\\delta','\\theta','\\lambda','\\mu','\\pi','\\sigma','\\omega',
    '\\partial','\\nabla','\\log','\\ln','\\sin','\\cos','\\tan',
    '\\vec','\\hat','\\bar','\\left','\\right','\\begin','\\end','\\text'];
  function hasMath(s) {
    for (let t = 0; t < TOKENS.length; t++) { if (s.indexOf(TOKENS[t]) !== -1) return true; }
    for (let i = 0; i < s.length; i++) { if (s[i] === '^' || s[i] === '_') return true; }
    return false;
  }
  const out = [];
  let i = 0;
  while (i < str.length) {
    if (str[i] === '\\' && i + 1 < str.length) {
      const nx = str[i+1];
      if (nx === '[' || nx === ']' || nx === '(' || nx === ')') { out.push('\\'); out.push(nx); i += 2; continue; }
    }
    if (str[i] === '[') {
      let k = i - 1;
      while (k >= 0 && str[k] === ' ') k--;
      if (k >= 4 && str[k]==='t' && str[k-1]==='f' && str[k-2]==='e' && str[k-3]==='l' && str[k-4]==='\\') { out.push('['); i++; continue; }
      let depth = 1, j = i + 1;
      while (j < str.length && depth > 0) { if (str[j]==='[') depth++; else if (str[j]===']') depth--; j++; }
      if (depth === 0) {
        const inner = str.slice(i+1, j-1);
        if (hasMath(inner)) { out.push('\\['); out.push(inner); out.push('\\]'); }
        else { out.push('['); out.push(inner); out.push(']'); }
        i = j;
...
async function send() {
  ...
  addMsg('user', _userBracketToLatex(q));
