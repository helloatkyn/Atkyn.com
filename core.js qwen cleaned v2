...
if (vvp) {
  ...
  // No VisualViewport API: keyboard compensation unavailable, but keep cadence
  // tracking consistent so window resize still invalidates the pipeline.
  window.addEventListener('resize', _onViewportEvent, { passive: true });
}
...
if (window.matchMedia) {
  ...
  const _onThemeChange = () => {
    ...
    // Invalidate bar size + spacer dedupe only. DO NOT clear _lastTransformStr:
    // if the keyboard is open the committed transform is already correct and
    // clearing it forces a redundant re-write that lands as a visible jump on
    // the frame the user taps the input right after a theme switch.
    // _geometryGeneration++ orphans any in-flight transition timer.
    _barHeightStale = true;
    _lastSpacerH    = -1;
    _geometryGeneration++;
    ...
    // Second commit two frames later catches late font/metric application.
    // Only mark bar stale — same reason: don't disturb a live transform.
    _raf(() => {
      _raf(() => {
        _barHeightStale = true;
        _lastSpacerH    = -1;
