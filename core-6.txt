...
if (pill && input) {
  pill.addEventListener('pointerdown', (e) => {
    ...
    /*
     * ROOT-CAUSE FIX — Plus button excluded from pill focus path.
     *
     * The previous guard returned early only when the target was NOT a button.
     * That meant a tap on plusBtn (a <button> inside/overlapping the pill area)
     * fell through to e.preventDefault() + input.focus() — opening the keyboard
     * on every Plus tap.
     *
     * Explicitly bail out when the tap originates from plusBtn or any child of
     * it. plusBtn has its own independent click handler and must never trigger
     * input focus.
     */
...
function _capturePlusMenuBaseStyles() {
  ...
  /* Do NOT capture bottom — we no longer write an inline bottom on the menu,
     so there is nothing to save or restore. Capturing it would cause
     _restorePlusMenuBaseStyles to write bottom:'', which can trigger a
     layout recalculation on siblings including chatbarWrap. */
  _plusMenuBaseStyles = {
...
function openPlusMenu() {
  ...
  /*
   * ROOT-CAUSE FIX — three changes here:
   *
   * 1. Removed getBoundingClientRect() — this forced a synchronous layout
   *    reflow on the composited layer, invalidating the chatbar paint and
   *    causing a visible glitch on open.
   *
   * 2. Removed plusMenu.style.bottom = computed — the inline bottom override
   *    was calculated from a now-stale vvp.height (race: the value changed
   *    between the read and the DOM write), and restoring it to '' on close
   *    (via _restorePlusMenuBaseStyles) triggered a layout recalculation that
   *    shifted chatbarWrap. Menu position is correctly owned by CSS.
   *
   * 3. Removed input.blur() — blurring the input when the keyboard is open
   *    causes the OS to dismiss the keyboard, which fires a visualViewport
   *    'resize' event → fixViewport() → _applyViewport() → chatbar transform
   *    + transition write. This is the primary cause of the bounce: the chatbar
   *    animated downward (keyboard close) while the menu was opening.
   *    The plus menu does not require the keyboard to be dismissed to function.
   */
