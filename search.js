...
function appendBotActions(msgEl, fullText) {
  ...
  const actions = [
    {
      key: 'copy', label: 'Copy',
      svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" width="19" height="19"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>`
    },
    {
      key: 'retry', label: 'Retry',
      svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" width="19" height="19"><path d="M3 7v6h6"/><path d="M21 17a9 9 0 0 0-9-9 9 9 0 0 0-6 2.3L3 13"/></svg>`
    },
    {
      key: 'like', label: 'Like',
      svg: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.6" stroke="currentColor" width="19" height="19"><path stroke-linecap="round" stroke-linejoin="round" d="M6.633 10.25c.806 0 1.533-.446 2.031-1.08a9.041 9.041 0 0 1 2.861-2.4c.723-.384 1.35-.956 1.653-1.715a4.498 4.498 0 0 0 .322-1.672V3a.75.75 0 0 1 .75-.75 2.25 2.25 0 0 1 2.25 2.25c0 1.152-.26 2.243-.723 3.218-.266.558.107 1.282.725 1.282m0 0h3.126c1.026 0 1.945.694 2.054 1.715.045.422.068.85.068 1.285a11.95 11.95 0 0 1-2.649 7.521c-.388.482-.987.729-1.605.729H13.48c-.483 0-.964-.078-1.423-.23l-3.114-1.04a4.501 4.501 0 0 0-1.423-.23H5.904m10.598-9.75H14.25M5.904 18.5c.083.205.173.405.27.602.197.4-.078.898-.523.898h-.908c-.889 0-1.713-.518-1.972-1.368a12 12 0 0 1-.521-3.507c0-1.553.295-3.036.831-4.398C3.387 9.953 4.167 9.5 5 9.5h1.053c.472 0 .745.556.5.96a8.958 8.958 0 0 0-1.302 4.665c0 1.194.232 2.333.654 3.375Z"/></svg>`
    },
    {
      key: 'dislike', label: 'Dislike',
      svg: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.6" stroke="currentColor" width="19" height="19"><path stroke-linecap="round" stroke-linejoin="round" d="M7.498 15.25H4.372c-1.026 0-1.945-.694-2.054-1.715a12.137 12.137 0 0 1-.068-1.285c0-2.848.992-5.464 2.649-7.521C5.287 4.247 5.886 4 6.504 4h4.016a4.5 4.5 0 0 1 1.423.23l3.114 1.04a4.5 4.5 0 0 0 1.423.23h1.294M7.498 15.25c.618 0 .991.724.725 1.282A7.471 7.471 0 0 0 7.5 19.75 2.25 2.25 0 0 0 9.75 22a.75.75 0 0 0 .75-.75v-.633c0-.573.11-1.14.322-1.672.304-.76.93-1.33 1.653-1.715a9.04 9.04 0 0 0 2.86-2.4c.498-.634 1.226-1.08 2.032-1.08h.384m-10.253 1.5H9.7m8.075-9.75c.01.05.027.1.05.148.593 1.2.925 2.55.925 3.977 0 1.487-.36 2.89-.999 4.125m.023-8.25c-.076-.365.183-.75.575-.75h.908c.889 0 1.713.518 1.972 1.368.339 1.11.521 2.287.521 3.507 0 1.553-.295 3.036-.831 4.398-.306.774-1.086 1.227-1.918 1.227h-1.053c-.472 0-.745-.556-.5-.96a8.95 8.95 0 0 0 .303-.54"/></svg>`
    },
    {
      key: 'refresh', label: 'Refresh',
      svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" width="19" height="19"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M8 16H3v5"/></svg>`
    }
  ];
...
function appendDisclaimer(msgEl) {
  ...
  disc.innerHTML =
    '<span>Atkyn is AI and can make mistakes.</span>';
...
async function send() {
  ...
  /* Hide disclaimer the moment user sends a new message */
  msgWrap.querySelectorAll('.ai-disclaimer').forEach(el => el.remove());
  ...
  try {
    ...
    /* ── Streaming bubble — created once, updated live ── */
    let streamBotEl = null;
    let streamBubbleEl = null;
    let streamAccum = '';       // accumulated raw text so far
    let streamRafPending = false;

    function _initStreamBubble() {
      if (streamBotEl) return;

      streamBotEl = document.createElement('div');
      streamBotEl.className = 'msg bot streaming';

      streamBubbleEl = document.createElement('div');
      streamBubbleEl.className = 'bubble';

      streamBotEl.appendChild(streamBubbleEl);

      /* Fade-in entry */
      streamBotEl.style.opacity = '0';
      streamBotEl.style.transform = 'translateY(6px)';
      streamBotEl.style.transition = 'opacity 0.22s ease-out, transform 0.22s ease-out';

      if (msgWrap) msgWrap.appendChild(streamBotEl);

      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (streamBotEl) {
            streamBotEl.style.opacity = '1';
            streamBotEl.style.transform = '';
          }
        });
      });
    }

    /* Throttled render — fires at most once per rAF tick */
    function _flushStreamRender() {
      streamRafPending = false;
      if (!streamBubbleEl) return;

      streamBubbleEl.innerHTML = _renderContent(streamAccum);
      _maybeScroll(streamBotEl, false);
    }

    function _onDelta(delta) {
      if (!delta) return;
      streamAccum += delta;

      /* Show the bubble on first token (typing indicator already removed) */
      if (!streamBotEl) _initStreamBubble();
    ...
    const fullText = await _parseSseStream(
      ...
      (results) => {
        ...
        if (!webCardShown) {
          ...
          /* Don't show typing again — stream bubble will appear on first delta */
          webCardShown = true;
        } else {
          _renderWebCards(results);
        }
    ...
    if (fullText) {
      ...
      /* ── Final render: citations + actions on the existing stream bubble ── */
      if (streamBotEl && streamBubbleEl) {
        /* Swap in fully-rendered HTML with citation chips */
        ...
        streamBubbleEl.innerHTML = html;
        streamBotEl.classList.remove('streaming');

        appendBotActions(streamBotEl, fullText);
        appendDisclaimer(streamBotEl);
        _injectCarousel();
        _maybeScroll(streamBotEl, false);

      } else {
        /* Fallback: stream bubble never appeared (0 deltas), render fresh */
