/* renderer.js — Atkyn Search
   ...
   [F31] Lone _ stripped with space · [F32] Lone * stripped with space
   [F33] Auto-space: digit→letter and word→.Capital merges fixed */
...
function _fmt(line, math) {
  ...
  // [F33] Auto-space: number immediately followed by letter (e.g. "309.35hai" → "309.35 hai")
  s = s.replace(/(\d)([a-zA-Z\u0900-\u097F])/g, '$1 $2');
  // [F33] Auto-space: letter/digit immediately followed by digit+letter merge after punctuation
  // e.g. "hai.Ye" → "hai. Ye"
  s = s.replace(/([a-zA-Z\u0900-\u097F])\.([A-Z\u0900-\u097F])/g, '$1. $2');
  s = s.replace(/([a-zA-Z\u0900-\u097F]),([a-zA-Z\u0900-\u097F])/g, '$1, $2');
  ...
  // Italic *text*
  s = s.replace(/(?<!\*)\*(?!\*)(?!\s)([^*\n]+?)(?<!\s)(?<!\*)\*(?!\*)/g, '<em>$1</em>');
  // [F32] Remove remaining lone asterisks with space so words don't merge
  s = s.replace(/(?<!\*)\*(?!\*)/g, ' ').replace(/ {2,}/g, ' ');

  // Italic _text_  [F28] [F30]
  s = s.replace(/(?<![_\w])_(?!_)(?!\s)([^_\n]+?)(?<!\s)(?<![_\w])_(?!_)/g, '<em>$1</em>');
  // [F31] Strip lone _ with space so adjacent words don't merge
  s = s.replace(/(?<!\w)_(?!\w)/g, ' ').replace(/ {2,}/g, ' ');
