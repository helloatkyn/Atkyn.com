...
function _buildWebCard(r) {
  ...
  // Favicon via Google's public favicon service (no auth required)
  const faviconSrc = `https://www.google.com/s2/favicons?sz=32&domain=${encodeURIComponent(hostname)}`;
...
function _renderWebCards(results) {
  ...
  if (carouselResults.length > 0) {
    ...
    msgWrap._pendingCarousel = carousel;
  }
}
...
function _injectCarousel() {
  const carousel = msgWrap._pendingCarousel;
  if (!carousel) return;
