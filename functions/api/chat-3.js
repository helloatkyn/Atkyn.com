...
const MAX_TOKENS_DIRECT_ANSWER = 600;  // Call 1 retry: when direct answer was cut off by token limit
  ...
  try {
    ...
    if (!call1Resp.ok) {
      ...
      if (msg?.tool_calls?.length) {
        ...
        // If the model hit the token limit on a direct answer, retry with a higher budget
        if (finishReason === 'length') {
          try {
            ...
            if (retryResp.ok) {
              ...
              // Retry failed — use the truncated answer as fallback
              directAnswer = msg.content.trim();
            }
          } catch {
            // Retry threw — use the truncated answer as fallback
            directAnswer = msg.content.trim();
          }
        } else {
          directAnswer = msg.content.trim();
