...
async function send() {
  ...
  try {
    ...
    const fullText = await _parseSseStream(
      reader,
      (results) => {
        /* Store sources for citation chips */
        _currentSources = results.map(r => ({ url: r.url, title: r.title, snippet: r.snippet || '' }));
    ...
    if (fullText) {
      ...
      bubbleEl.innerHTML = injectCitationChips(renderMarkdown(fullText), _currentSources);
