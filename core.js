...
let _currentTabKey = (() => {
  ...
  /* Restore tab selected before a reload-on-switch */
  try {
    const pending = sessionStorage.getItem('atkyn_pending_tab');
    if (pending) {
      sessionStorage.removeItem('atkyn_pending_tab');
      const pendingEl = tabBar.querySelector(`.tab[data-tab="${pending}"]`);
      if (pendingEl) {
...
if (tabBar) {
  tabBar.addEventListener('click', (e) => {
    ...
    try {
      sessionStorage.setItem('atkyn_pending_tab', key);
    } catch (_) {}
