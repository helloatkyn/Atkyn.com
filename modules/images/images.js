(function () {
  ...
  // Based on the sugg-pill min-height (48px) + two 9px gaps = ~66px. Use 72 for comfort.
  const MIN_GAP_PX = 72;
  ...
  // Uses a WeakSet to coalesce multiple rapid calls for the same element.
  function _scheduleMeasure(gridSection) {
    if (_pendingMeasure.has(gridSection)) return;
    _pendingMeasure.add(gridSection);
    requestAnimationFrame(function () {
      // _pendingMeasure is WeakSet, no explicit delete needed; next call re-adds
  ...
  function _measureAndFill(gridSection) {
    ...
    // How many pills can fit? Each pill is at least 48px tall plus a 9px gap.
    // Don't stack more pills than the available space comfortably holds.
    const pillSlotHeight = 48 + 9; // min-height + gap
    const maxPills = Math.floor((gap - 9) / pillSlotHeight); // subtract one gap for bottom
  ...
  function _onGalleryResize() {
    if (!_gallery) return;
    // Reset pool from the master suggestions list minus what's still in fixed sugg-rows
