...
scroll · header animation · keyboard positioning · theme reload
...
/* ════════════════════════════════
THEME CHANGE → PAGE RELOAD
════════════════════════════════ */

/*
 * Theme is OS-controlled via prefers-color-scheme. When the OS switches,
 * reload the page so the keyboard / VisualViewport system initialises
 * cleanly under the new theme.
 *
 * No reload loop risk: the 'change' event only fires on an actual OS
 * color-scheme change, never on page load.
 */
if (window.matchMedia) {
  const themeMQ = window.matchMedia('(prefers-color-scheme: dark)');

  const _onThemeChange = () => { location.reload(); };
