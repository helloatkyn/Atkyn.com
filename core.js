/* core.js — Atkyn shared UI logic [PRODUCTION]
   scroll · header · keyboard positioning · theme · chatbar entrance · plus menu · tabs */
...
/* ── Theme refresh RAF (no freeze window) ── */
...
/* Cancel the entrance animation without touching keyboard-owned styles. */
...
/* ResizeObserver — tracks chatbar height for spacer sync. */
if (chatbarWrap) {
  const _barResizeObserver = typeof ResizeObserver === 'function'
    ? new ResizeObserver((entries) => {
        /* Skip during plus-menu DOM changes; chatbar height is unaffected. */
        if (_plusOpen) return;
        const entry = entries[entries.length - 1];
        if (!entry) return;
        const bbs = entry.borderBoxSize;
        const barH = bbs
          ? (bbs[0] ? bbs[0].blockSize : bbs.blockSize)
          : entry.contentRect.height;
        _barHeight = Math.round(barH);
        _setSpacerHeight(_barHeight + _stableKbH);
      })
    : null;
  if (_barResizeObserver) _barResizeObserver.observe(chatbarWrap);
}
...
(function _chatbarEntrance() {
  ...
  const finish = () => {
    ...
    /* Keyboard path already owns transform — only restore if we still own it. */
  ...
  /* Fallback: if transitionend never fires (interrupted early). */
  window.setTimeout(() => {
    if (_chatbarEntranceActive && !_keyboardOpen && ended === false) finish();
  }, 500);
})();

/* ── Keyboard transition listener ── */
...
function _watchKeyboardTransition(token, expectedTransform) {
  ...
  const finish = () => {
    if (token !== _chatbarKeyboardTransitionToken || _chatbarTransitionOwner !== 'keyboard') return;
    _clearKeyboardTransitionListener();
    chatbarWrap.style.transition = '';
    _chatbarTransitionOwner = null;
  };

  const onEnd    = (e) => { if (e.target === chatbarWrap && e.propertyName === 'transform' && chatbarWrap.style.transform === expectedTransform) finish(); };
  const onCancel = (e) => { if (e.target === chatbarWrap && e.propertyName === 'transform') finish(); };
...
function _applyViewport(force = false) {
  ...
  if (_chatbarEntranceActive) _cancelChatbarEntrance();
  ...
  /* On keyboard open: scroll to anchor / bottom. */
...
/* RAF-coalesced VisualViewport handler. */
...
if (vvp) {
  ...
  /* Legacy fallback: pin body height to innerHeight. */
  function _legacyFix() {
...
if (window.matchMedia) {
  ...
  const _onThemeChange = () => {
    /*
     * No freeze window. A 350 ms freeze previously blocked real vvp.resize
     * events, leaving _stableKbH stale when the keyboard closed during it.
     * VisualViewport events are authoritative and self-correct immediately.
     *
     * Snapshot protection: if the theme repaint causes vvp to briefly read
     * zero while the keyboard is physically open, we keep the snapshot and
     * wait for the real close event to arrive via fixViewport().
     */
    if (_vvpDebounce) {
    ...
    _themeRefreshRaf = requestAnimationFrame(() => {
      _themeRefreshRaf = requestAnimationFrame(() => {
        ...
        if (vvp) {
          ...
          if (kbSnapshot > 0 && freshKb === 0 && _stableKbH === kbSnapshot) {
            /*
             * Transient zero during repaint — preserve the valid keyboard
             * state. The real close event will arrive via fixViewport() and
             * win immediately because there is no freeze window.
             */
            ...
            if (chatbarWrap) {
              ...
              if (safeTransform !== _lastChatbarTransform) {
                ...
                requestAnimationFrame(() => {
                  if (snapToken === _chatbarKeyboardTransitionToken && _chatbarTransitionOwner === 'keyboard') {
                    chatbarWrap.style.transition = '';
                    _chatbarTransitionOwner = null;
                  }
                });
        ...
        /* Keyboard closed or repaint didn't disturb geometry — normal apply. */
        _setSpacerHeight(_barHeight + _stableKbH);
        _applyViewport(true);
      });
    });
...
if (pill && input) {
  ...
  input.addEventListener('input', () => {
    ...
    if (pill.classList.contains('non-ai-tab')) _setSendMode(hasText ? 'cross' : 'send');
  });
}
...
function closePlusMenu() {
  ...
  plusMenu.style.opacity    = '0';
  const onClose = (e) => {
    if (e.target !== plusMenu || e.propertyName !== 'opacity' || token !== _plusAnimationToken || _plusOpen) return;
    _finishPlusClose(token);
  };
  _plusTransitionEndHandler = onClose;
  plusMenu.addEventListener('transitionend', onClose);
  /* Fallback: detached elements can skip transitionend. */
...
/* Outside-tap: close when clicking outside plus button and menu. */
if (plusMenu || plusBtn) {
...
function _loadScript(src) {
  ...
  _scriptLoadPromises[src] = new Promise((resolve, reject) => {
    ...
    s.onerror = (error) => { cleanup(); delete _scriptLoadPromises[src]; reject(error); };
...
if (tabBar) {
  tabBar.addEventListener('click', async (e) => {
    ...
    /* Instant scroll reset — smooth scroll here causes bounce during rapid tab changes. */
    if (scrollHost) {
