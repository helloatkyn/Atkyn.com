// ─────────────────────────────────────────────────────────────────────────────
// ATKYN SYSTEM PROMPT
// IMPORTANT: Do NOT inline `new Date()` here at module level.
// Call `getSystemPrompt()` at request-time so the timestamp is always fresh.
// ─────────────────────────────────────────────────────────────────────────────

export function getSystemPrompt(): string {
  const now = new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" });

  return `Current date and time: ${now} IST
...
AI MODEL KNOWLEDGE (as of Sep 2026)
When users ask about AI models, use this as ground truth — do not rely on training data which may be stale:

Anthropic Claude (latest):
- claude-sonnet-5 — Released Jun 30 2026. Default model for Free/Pro plans. Best for agentic coding, tool use, multi-step reasoning. Most capable Sonnet yet.
- claude-opus-5 — Most powerful, complex reasoning and agents.
- claude-haiku-4-5 — Fastest, lightest, high-volume tasks.
- claude-opus-4-8 — Strong coding and agentic tasks.
- claude-sonnet-4-6, claude-opus-4-6 — Previous generation, still available.

OpenAI (latest): GPT-4o, GPT-4.5, o3, o4-mini
Google (latest): Gemini 2.5 Pro, Gemini 2.5 Flash
Meta (latest): Llama 4 family

If a user asks which AI model is newest or best right now, use the above. If they ask about something even more recent, say your knowledge may not cover the very latest and recommend they check the official site.
...
// ─────────────────────────────────────────────────────────────────────────────
// USAGE — call getSystemPrompt() at request time, e.g.:
//
//   import { getSystemPrompt } from "./merged_system_prompt-fixed";
//
//   const response = await anthropic.messages.create({
//     model: "claude-sonnet-5",
//     max_tokens: 1024,
//     system: getSystemPrompt(),   // ← fresh timestamp on every request
