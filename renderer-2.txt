/* ═══════════════════════════════════════════════════════════════
   ...
   AUDIT FIXES (v6):
   ─────────────────
   [F30] Inline code placeholder (\x03C{N}\x03) now restored AFTER
         bold/italic processing in _fmt(), not before. Previously the
         restore happened first, so ** inside a backtick span (e.g.
         `**not bold**`) would be converted to <strong> by the bold
         regex running on the already-restored <code>**not bold**</code>.

   [F31] Markdown link URL regex upgraded to allow one level of balanced
         parentheses — ((?:[^)(]|\([^)]*\))+) — so Wikipedia-style URLs
         like /wiki/C_(programming_language) are no longer truncated at
         the first ) leaving a dangling ) in the rendered text.
   ═══════════════════════════════════════════════════════════════ */
...
function _fmt(line, math) {
  ...
  // Restore inline code placeholders AFTER bold/italic — prevents ** inside   [F30]
  // `backtick spans` from being parsed as bold by the bold regex above.
  s = s.replace(/\x03C(\d+)\x03/g, (_, i) => codePh[+i]);

  // Markdown links [text](url)                                    [F13]
  // [F31] URL regex allows one level of balanced parens so Wikipedia-style
  // URLs like /wiki/C_(programming_language) are not truncated at the first ).
