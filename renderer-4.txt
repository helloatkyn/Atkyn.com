/* ═══════════════════════════════════════════════════════════════
   ...
   AUDIT FIXES (v6 — ITALIC LEAKAGE FIX):
   ─────────────────────────────────────────
   [F30] ROOT CAUSE of unexpected italic/cursive rendering — FIXED.

         The italic regex in _fmt() was:
           /(?<!\*)\*(?!\*)([^*\n]+?)(?<!\*)\*(?!\*)/g

         Missing constraints: no check that the opening * is NOT
         followed by whitespace, and no check that the closing * is
         NOT preceded by whitespace. This violates CommonMark §6.2
         emphasis rules and causes the lazy quantifier [^*\n]+? to
         greedily consume text from a stray/unmatched * all the way
         to the next valid-looking * on the same line. Examples:

           "text *unit. More *word*" (AI uses * for footnote/unit)
           → was:  text <em>unit. More </em>word  ← italic leakage
           → now:  text unit. More <em>word</em>  ← correct

           "*$3T. Yeh *bohot* hai" (stray * before currency symbol)
           → was:  <em>$3T. Yeh </em>bohot hai  ← whole phrase italic
           → now:  $3T. Yeh <em>bohot</em> hai  ← correct

         THE FIX: add (?!\s) immediately after opening * and (?<!\s)
         immediately before closing *. These are the whitespace
         boundary rules required by the CommonMark spec:
           - opening delimiter: must NOT be followed by Unicode space
           - closing delimiter: must NOT be preceded by Unicode space

         New regex:
           /(?<!\*)\*(?!\*)(?!\s)([^*\n]+?)(?<!\s)(?<!\*)\*(?!\*)/g

         Same fix applied to the underscore italic regex [F28].

         No valid italic is affected: *italic* and *multi word italic*
         both still match correctly — their opening * is always
         followed by a non-space char and closing * is preceded by
         a non-space char.

         This fix does NOT touch fonts, CSS, layout, API, search,
         streaming protocol, or any other rendering path.
   ═══════════════════════════════════════════════════════════════ */
...
function _fmt(line, math) {
  ...
  // [F30] Added (?!\s) after opening * and (?<!\s) before closing * (CommonMark
  //       emphasis rules). This prevents a stray/unmatched * followed or preceded
  //       by whitespace from greedily opening an italic span that consumes text
  //       up to the next valid-looking * on the same line, e.g.:
  //         "text *unit. More *word*" → was: <em>unit. More </em>word
  //         After fix:               → "text unit. More <em>word</em>"
  s = s.replace(/(?<!\*)\*(?!\*)(?!\s)([^*\n]+?)(?<!\s)(?<!\*)\*(?!\*)/g, '<em>$1</em>');
  ...
  // [F30] Same whitespace boundary constraints as * italic above.
