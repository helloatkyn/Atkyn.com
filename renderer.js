...
function _buildChip(src, allSources) {
  ...
  /* Embed snapshot of sources at chip-build time — survives future searches */
...
function injectCitationChips(html, sources) {
  ...
  return html.replace(/((?:\[\d+\])+)([.,;:!?]*)/g, function(_, refs, punct) {
    ...
    const re = /\[(\d+)\]/g;
    let m;
    while ((m = re.exec(refs)) !== null) nums.push(parseInt(m[1], 10));

    const unique = [...new Set(nums)];
    let inner = '';
    /* Show only the first unique source — no overflow chip */
    const src = sources[unique[0] - 1];
    if (src) inner = _buildChip(src, sources);

    return inner ? '<span class="chip-group">' + inner + '</span>' + punct : refs + punct;
  });
}
...
(function () {
  ...
  document.addEventListener('click', function (e) {
    ...
    if (!chip) return;
    e.preventDefault();
    /* Use sources snapshot embedded in chip — not the global (which may be stale) */
    let chipSources = null;
    try { chipSources = JSON.parse(chip.dataset.chipSources || 'null'); } catch (_) {}
