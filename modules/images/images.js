(function () {
  ...
  /* ── Suggestions fetch (frontend-only) ──────────────────────
     Primary:  DuckDuckGo /ac/ — CORS-open (Access-Control-Allow-Origin: *),
               no credentials, returns a flat JSON array of strings.
     Fallback: Wikipedia OpenSearch — also CORS-open via origin=* param,
               returns OpenSearch format ["query", ["title1",...], ...].

     Google suggestqueries.google.com is NOT used: it does not send
     Access-Control-Allow-Origin headers for arbitrary origins, so every
     browser fetch() from a non-Google origin is blocked by CORS policy.

     No backend proxy. No API key. No secrets.
  ─────────────────────────────────────────────────────────────── */

  // Per-query session cache so repeated identical queries don't re-fetch.
  const _suggCache = Object.create(null);

  // Parse and clean a raw suggestion list against the original query.
  ...
  function _fetchSuggestions(q, token) {
    ...
    // Primary: DuckDuckGo autocomplete
    // Returns a flat JSON array: ["suggestion1", "suggestion2", ...]
    ...
    fetch(ddgUrl)
      ...
      .then(function (data) {
        if (token !== _queryToken) return;
        // DDG returns a flat array of strings
        if (!Array.isArray(data) || data.length === 0) throw new Error('ddg-empty');
        const fresh = _cleanSuggestions(data, q);
        if (!fresh.length) throw new Error('ddg-empty');
        _suggCache[q] = fresh;
        _applySuggestions(fresh, token);
      })
      .catch(function () {
        // Primary failed — try Wikipedia OpenSearch as fallback
        if (token !== _queryToken) return;
        const wikiUrl = 'https://en.wikipedia.org/w/api.php'
          + '?action=opensearch&search=' + encodeURIComponent(q)
          + '&limit=12&format=json&origin=*';
        ...
        fetch(wikiUrl)
          ...
          .then(function (data) {
            if (token !== _queryToken) return;
            // Wikipedia OpenSearch: ["query", ["title1","title2",...], ...]
            if (!Array.isArray(data) || !Array.isArray(data[1])) return;
            const fresh = _cleanSuggestions(data[1], q);
            if (fresh.length) {
              _suggCache[q] = fresh;
              _applySuggestions(fresh, token);
            }
          ...
          .catch(function () {
            // Both sources failed — images continue normally, no error shown.
            if (token !== _queryToken) return;
            _suggestions = [];
            _suggPool = [];
          });
