(function () {
  ...
  function _renderGallery(results) {
    ...
    const SUGG_AT = new Set([2, 5, 9, 14, 19]);

    const injectSuggestions = function () {
      if (!suggQueue.length) return;
      const count = suggQueue.length === 1 ? 1 : Math.min(suggQueue.length, 4);
    ...
    while (queue.length > 0) {
      if (SUGG_AT.has(sectionNumber)) injectSuggestions();
