/* ═══════════════════════════════════════════════════════════════
   ...
   AUDIT FIXES (v6):
   ─────────────────
   [F30] _fmt(): unwrap <em> tags that wrap only numbers, symbols, or short
         numeric strings — Mistral Large wraps numbers like 1.95, 0.63%,
         312.05 in italic stars causing garbled output. Real emphasis (3+
         alpha chars, not purely numeric/punctuation) is preserved.
...
function _fmt(line, math) {
  ...
  // [F30] Strip stray em/strong tags that Mistral produces around numbers/units
  // e.g. <em>1.95 points or 0.63%</em><strong>312.05</strong> → plain text
  // Unwrap <em> that contains only numbers, punctuation, or short mixed text
  // (real emphasis is rare in stock/search answers; this prevents garbled output)
  s = s.replace(/<em>([^<]{1,80})<\/em>/g, (_, inner) => {
    // Keep em only if inner looks like real emphasis (words, not numbers/symbols)
    const looksLikeEmphasis = /[a-zA-Z]{3,}/.test(inner) && !/^[\d\s.,!?%$+\-:;()]+$/.test(inner);
    return looksLikeEmphasis ? `<em>${inner}</em>` : inner;
  });
