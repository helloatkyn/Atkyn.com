...
function _applyViewport() {
  ...
  // kbHeight = space between bottom of visual viewport and bottom of layout viewport.
  ...
  // address-bar noise only (keyboard state unchanged) — bail out.
  // Chatbar is fixed at bottom:0, must not move during normal scroll.
  if (!_keyboardOpen && !wasOpen) return;

  // Use `bottom` instead of `translateY` — correct for position:fixed elements.
  // translateY on a fixed element fights Android Chrome keyboard-avoidance
  // and is what caused the visible bounce/jump during scroll.
  const effectiveKb = _keyboardOpen ? kbHeight : 0;
  chatbarWrap.style.transition = (_keyboardOpen && !wasOpen)
    ? 'none'
    : 'bottom 0.28s cubic-bezier(0.0, 0.0, 0.2, 1)';
  chatbarWrap.style.transform = '';
  chatbarWrap.style.bottom    = effectiveKb > 0 ? effectiveKb + 'px' : '0';
  updateSpacer(effectiveKb);
