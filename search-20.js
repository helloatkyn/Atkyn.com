...
function _renderStockCard(data) {
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      _initStockChart(chartContainer, data.series, isUp);
      scrollToMsg(wrap);
    });
  });
}
...
async function _fetchAndUpdateChart(ticker, range, container, data) {
  ...
  try {
    ...
    // We need the Finnhub key — we'll proxy through our /api/stockcandle endpoint
    ...
    container.innerHTML = '';
    _initStockChart(container, series, up);
  } catch {
    // Fallback: redraw with existing 1D data
    container.innerHTML = '';
    _initStockChart(container, data.series, data.change >= 0);
  }
}

function _initStockChart(container, series, isUp) {
  const LW = window.LightweightCharts;
  if (!LW || !series || series.length === 0) {
